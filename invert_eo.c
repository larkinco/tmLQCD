...
int invert_eo(spinor * const Even_new, spinor * const Odd_new, 
  ...
  if(even_odd_flag) {
    ...
    if(solver_flag == BICGSTAB) {
      ...
      Qtm_minus_psi(Odd_new, Odd_new);
       ...
       iter = arpack_cg(VOLUME/2,solver_params.arpackcg_nrhs,solver_params.arpackcg_nrhs1, Odd_new, g_spinor_field[DUM_DERI],&Qtm_pm_psi,
                              ...
                              solver_params.arpackcg_start_vec_opt,solver_params.arpackcg_start_vec_fname,
                              solver_params.arpackcg_comp_evecs,solver_params.use_acc,
    ...
    else if(solver_flag == RELIABLECG) {
       /* Here we invert the hermitean operator squared */
       gamma5(g_spinor_field[DUM_DERI], g_spinor_field[DUM_DERI], VOLUME/2);  
       if(g_proc_id == 0) {printf("# Using RELIABLECG!\n"); fflush(stdout);}
       iter = reliable_cg_her(Odd_new, g_spinor_field[DUM_DERI], max_iter, precision, rel_prec, VOLUME/2, &Qtm_pm_psi,solver_params.cg_reliable_update_freq);
       Qtm_minus_psi(Odd_new, Odd_new);
    }
